...
def get_table(tablename, condition = None, sort = None):
    cursor = get_cursor()
    try:
        if not condition and not sort:
            ...
            result = [a for a in cursor.execute("select * from " + tablename + " order by " + sort)]
            cursor.close()
            return result
        else:
            result = [a for a in cursor.execute("select * from " + tablename+ " where "+ condition + " order by "+ sort)]
            cursor.close()
            return result
